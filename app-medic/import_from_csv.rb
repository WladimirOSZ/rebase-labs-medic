...
res = rows.map do |row|
  ...
  if exam.include?(exam_token)
    ...
    unless patient.include?(patient_cpf)
      result = conn.exec_params('INSERT INTO patients 
    ...
    unless doctor.include?(doctor_crm)
      result = conn.exec_params('INSERT INTO doctors (crm, crm_state, name, email) 
                                 VALUES ($1, $2, $3, $4) 
                                 RETURNING id',
    ...
    result = conn.exec_params('INSERT INTO exams (patient_id, doctor_id, token, date) 
                               VALUES ($1, $2, $3, $4) 
                               RETURNING id',
